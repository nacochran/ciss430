...
private:
    ...
    void evictFrame(int frame_id) {
	  ...
	  int getLRUFrame() {
				// Start from the least recently used frame (back of the LRU list)
...
public:
    ...
    void fetchPage(int page) {
        ...
        if (frame_id == -1) {
						...
						if (frame_id == -1)
						{
								std::cout << "All blocks are being utilized. Operation cannot be performed.\n";
								return;
						}
						else
						{
            ...
            if (option == 0) {
                ...
                std::cin >> page_id;
                    ...
                    std::cout << "Invalid page number.\n";
										...
										if (frame_id == -1)
										{
												std::cout << "Page not in cache memory.\n";
										} else {
												pins[frame_id] += 1;
										}
								}
                    ...
                    std::cout << "Invalid page number.\n";
										...
										if (frame_id == -1)
										{
												...
												{
														std::cout << "Pin # for page is already at 0.\n";
												}
										}
								}
						}
